...
for coef in [1e-1, 1, 1e1]:
  filename = 'polyG10Coef' + str(coef) + '.out'
  file = open(filename, 'w')
  for C in [1e-3, 1e-2, 1e-1, 1, 1e1, 1e2, 1e3, 1e4]:
    clf = svm.SVC(kernel = 'poly', C = C, degree=3, gamma=10, coef0=coef).fit(tr, trl)
    ...
    file.write(str(etr * 100)); file.write(str(' '))
    file.write(str(edv * 100)); file.write(str(' '))
    file.write('\n')
  file.close()
